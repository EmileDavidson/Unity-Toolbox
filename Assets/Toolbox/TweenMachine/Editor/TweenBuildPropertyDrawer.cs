...
namespace Toolbox.TweenMachine.Editor
{
    ...
    private void Draw(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (property.isExpanded)
        {
            ...
            _defaultFoldout = EditorGUI.Foldout(new Rect(position.x + 8, position.y + _totalPropertyHeight + _standardPropertyHeight, position.width, _standardPropertyHeight), _defaultFoldout, "Default values");
            _totalPropertyHeight += _standardPropertyHeight;
            if (_defaultFoldout) DefaultTweenValueDrawer();
            
            
            _tweenFoldout = EditorGUI.Foldout(new Rect(position.x + 8, position.y + _totalPropertyHeight + _standardPropertyHeight, position.width, _standardPropertyHeight), _tweenFoldout, "Tween values");
            _totalPropertyHeight += _standardPropertyHeight;
            if (_tweenFoldout) TweenValueDrawer();
            
            _positionFoldout = EditorGUI.Foldout(new Rect(position.x + 8, position.y + _totalPropertyHeight + _standardPropertyHeight, position.width, _standardPropertyHeight), _positionFoldout, "Position values");
            _totalPropertyHeight += _standardPropertyHeight;
            if (_positionFoldout) PositionValueDrawer();
            
            _rotationFoldout = EditorGUI.Foldout(new Rect(position.x + 8, position.y + _totalPropertyHeight + _standardPropertyHeight, position.width, _standardPropertyHeight), _rotationFoldout, "Rotation values");
            _totalPropertyHeight += _standardPropertyHeight;
            if (_rotationFoldout) RotationValueDrawer();
            
            _scaleFoldout = EditorGUI.Foldout(new Rect(position.x + 8, position.y + _totalPropertyHeight + _standardPropertyHeight, position.width, _standardPropertyHeight), _scaleFoldout, "Scale values");
            _totalPropertyHeight += _standardPropertyHeight;
            if (_scaleFoldout) ScaleValueDrawer();
            
            _colorFoldout = EditorGUI.Foldout(new Rect(position.x + 8, position.y + _totalPropertyHeight + _standardPropertyHeight, position.width, _standardPropertyHeight), _colorFoldout, "Color values");
            _totalPropertyHeight += _standardPropertyHeight;
    ...
    private void DefaultTweenValueDrawer()
    {
        ...
        EditorGUI.PropertyField(new Rect(_position.x + 16, _position.y + _totalPropertyHeight + _standardPropertyHeight, _position.width - 8, _standardPropertyHeight), _property.FindPropertyRelative("onTweenBuildStart"));
        _totalPropertyHeight += (_standardPropertyHeight * 5) + 2;
    }
